...
RSpec.describe Force::SoqlQueryBuilder do
  describe '#to_soql' do
    ...
    it 'should generate soql' do
      ...
      expect(soql).to eq("SELECT Id FROM Application__c WHERE (Status__c != 'DRAFT') LIMIT 10000 OFFSET 20000")
    end
    ...
    it 'should allow multiple wheres' do
      ...
      expect(soql).to eq("SELECT Id FROM Application__c WHERE (Status__c != 'DRAFT') AND (Id == 1) LIMIT 10000 OFFSET 20000")
    end
