...
describe('SupplementalApplicationPage Rental Assistance Information section', () => {
  test(
    'should allow a new rental assistance to be created',
    async () => {
      ...
      const prevTableSize = await page.$$eval(
        '.rental-assistances > tbody > .tr-expand',
        (elems) => elems.length
      )
      ...
      await page.waitForSelector('#rental-assistance-save')

      // Save the new rental assistance form
      await page.click('#rental-assistance-new-form #rental-assistance-save')
      ...
      const newTableSize = await page.$$eval(
        '.rental-assistances > tbody > .tr-expand',
        (elems) => elems.length
      )
      ...
      const newRecipient = await page.$eval(
        `${lastRentalAssistanceSelector} td:nth-child(1)`,
        (e) => e.textContent
      )
      const newTypeLabel = await page.$eval(
        `${lastRentalAssistanceSelector} td:nth-child(2)`,
        (e) => e.textContent
      )
      const newAmount = await page.$eval(
        `${lastRentalAssistanceSelector} td:nth-child(3)`,
        (e) => e.textContent
      )
      const newRecurring = await page.$eval(
        `${lastRentalAssistanceSelector} td:nth-child(4)`,
        (e) => e.textContent
      )
  ...
  test(
    'should allow a rental assistance to be updated',
    async () => {
      ...
      await page.waitForSelector('#rental-assistance-edit-form-0')
      ...
      await page.click('#rental-assistance-save')
      ...
      const newAmount = await page.$eval(
        `${firstRentalAssistanceSelector} td:nth-child(3)`,
        (e) => e.textContent
      )
      expect(newAmount).toEqual('$1,101.00')

      await page.click(`${firstRentalAssistanceSelector} button.action-link`)

      const newAmountValue = await page.$eval('#assistance_amount', (e) => e.value)
      expect(newAmountValue).toEqual('$1,101.00')
    },
    DEFAULT_E2E_TIME_OUT
  )
  ...
  test(
    ...
    async () => {
      ...
      await sharedSteps.enterValue(page, '#assistance_amount', '$1102')

      // Save the edit rental assistance form
      await page.click('#rental-assistance-edit-form-0 #rental-assistance-save')
  ...
  // succeeded, then there will be at least one rental assistance present.
  test(
    'should allow a rental assistance to be deleted',
    async () => {
      const { page } = await SetupBrowserAndPage(testBrowser)
      ...
      await page.click('#rental-assistance-delete')
